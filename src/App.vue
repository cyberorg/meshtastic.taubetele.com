<template>
  ...
  <modal v-if="shouldShowInfoModal" title="Info" @close="handleInfoClose">
    ...
    <div v-else>
      ...
      <br />
      <p>
        Данная карта отображает публичные данные устройств входяцие в данныую
        радиосеть. Для того чтобы они появились тут вы можете настроить MQTT на
        своеей ноде или просто добраться до ближайше сети в которой есть уже
  ...
  >
    ...
    <div v-else>
      ...
      <div
        class="p-2 transition-colors hover:bg-neutral-300"
          ...
          <table>
            <tbody>
              <tr>
                ...
                <td>
                  <div
                    @click="addToFilter(devices[device].server)"
                    class="w-full text-sm text-blue-500 cursor-pointer w-fit"
                  >
              ...
              <tr v-if="devices[device]?.user?.rxSnr">
                <td>RX SNR</td>
                <td>
                  {{ devices[device].user.rxSnr }} <br />
                  уровень сигнала с которым пришел пакет nodeinfo
                </td>
              </tr>
              <tr v-if="devices[device]?.user?.rxRssi">
                <td>RX RSSI</td>
                <td>
                  {{ devices[device].user.rxRssi }} <br />
                  уровень сигнала с которым пришел пакет nodeinfo
                </td>
              </tr>
              <tr
              ...
              <tr
                v-if="
                  devices[device]?.environmentMetrics?.data?.deviceMetrics
                    ?.barometricPressure
                "
              >
              ...
              <tr v-if="devices[device]?.mqtt">
                <td><div class="font-bold">MQTT:</div></td>
                <td><div class="font-bold">Online</div></td>
              </tr>
              <tr v-if="devices[device]?.mqtt">
                <td>Server:</td>
                <td>{{ devices[device]?.server }}</td>
              </tr>
              <tr
                v-if="
                  devices[device]?.text?.data || devices[device]?.message?.data
                "
              >
                <td>Last public message:</td>
                <td>{{ devices[device]?.message?.data }}</td>
              </tr>
            </tbody>
          </table>
        </div>
